<!-- ws:dev-reference source=wrightstrategy/bridge:conventions/dev-reference.md version=1 sha256=22866434272b078b653c5698d47f00e919faf00545453cb5cea3954a6bfe5bad -->
...
does not repeat those same build/test/scan jobs. Remove redundant main-push work when the required
PR/merge-queue checks already own it and there is no separate post-merge obligation.

For Graphify adopters, heavy default-branch build/test/scan/deploy work must exclude pushes changing
only `graphify-out/**`. A refresh updates the agent’s map, not the application. Select by changed
inputs, not actor identity or commit-message skip markers; other bot pushes can contain real source
changes. Prefer the smallest pattern that fits the workflow:

- **Push-only exclusion:** add `graphify-out/**` to the existing `push.paths-ignore` list when
  every non-graph change should retain its current behavior. Preserve existing branch/tag selectors
  and exclusions. For example, this main-only push event leaves required PR/merge-group events
  unfiltered:
...
- **Positive source paths:** keep an already-correct `push.paths` list that covers every real build
  input (source, lockfiles, Dockerfiles, build scripts/configuration and the builder workflow).
  Graph output must not match it. Do not add `paths-ignore` alongside `paths` for the same event;
  if exclusions are needed, use ordered negative patterns within `paths`.
- **Existing job/step classification:** retain a workflow that must report required checks and
  skip only irrelevant heavy work through its existing change detector. Required aggregate checks
  must still report accurately; unavailable comparison evidence must select required work rather
  than silently treating the change as graph-only. A runner start for classification is acceptable.

Source-only and mixed source/graph pushes must still select relevant heavy work. Never path-filter
an entire required PR workflow: GitHub leaves its absent checks pending. Preserve required
merge-group reporting too. Lightweight intentional checks and release bookkeeping are not
inherently heavy CI; inspect the selected work before declaring a defect.

**Release boundary:** GitHub does not evaluate path filters for tag pushes, but excluding a branch
build can still leave a later tag without its required exact-commit image. Before changing triggers,
verify the owner’s release-target selection and image availability together, including a release
after a graph refresh. Preserve tag, manual and maintenance-branch behavior and the retag-only
release contract; do not relabel an ancestor image as if it were built from the skipped SHA. A
release flow that requires an image for every branch tip needs a coordinated owner-repo repair,
not a filter-only edit. Existing concurrency protection for source builds remains necessary.

Verify graph-only, source-only and mixed pushes, required PR/merge-group checks, and retained
release/manual paths when changing a trigger. Native path filters are the standard small-refresh
pattern, not an absolute guarantee for arbitrary pushes: GitHub bounds the compared-file list;
very large pushes or diff-generation timeouts can run the workflow unconditionally. See
[GitHub’s path-filter semantics](https://docs.github.com/en/actions/reference/workflows-and-actions/workflow-syntax#onpushpull_requestpull_request_targetpathspaths-ignore).

For workflows that produce an artifact from each selected shipped commit, combine a per-ref PR
group with cancellation and a per-SHA default-branch group:
...
- **Trigger matrix:**
  ...
  - `v*` tag (from the repo's release workflow — see Versioning & releases — never by hand) →
    ...
    **append the `sha256:` index digest to the GitHub Release body**. No rebuild, no re-scan. A
    ...
    Graphify adopters excluding graph-only main pushes must coordinate release-target selection
    with that exclusion; see the [one-run-per-change rule](#concurrency-and-merge-queues).
...
marked and its protection is ready. Consumer heavy CI must also exclude graph-only refreshes under
the existing [one-run-per-change rule](#concurrency-and-merge-queues); the refresh workflow’s own
self-exclusion does not configure other workflows.
